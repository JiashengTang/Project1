...
<html lang="en">
   ...
   <body class="m--skin- m-header--fixed m-header--fixed-mobile m-aside-left--enabled m-aside-left--skin-dark m-aside-left--fixed m-aside-left--offcanvas m-footer--push m-aside--offcanvas-default">
      ...
      <div class="m-grid m-grid--hor m-grid--root m-page">
         <div class="m-grid__item m-grid__item--fluid m-grid m-grid--hor m-login m-login--signin m-login--2 m-login-2--skin-1" id="m_login" style="background-image: url(resources/images/login_bg.jpg);">
            <div class="m-grid__item m-grid__item--fluid m-login__wrapper">
               <div class="m-login__container">
                  <div class="m-login__logo">
                     <a href="#">
                        <img src="{{url('resources/images/logo.png')}}">
                     </a>
                  </div>
                  <div class="m-login__signin">
                     <div class="m-login__head">
                        <h3 class="m-login__title">Sign In To JobPast</h3>
                     </div>
                    <form class="m-login__form m-form" method="post" enctype="multipart/form-data">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        @if(isset($showErrorMsg))
                           <div class="form-group m-form__group">
                              <p style="text-align: center;"><span class="m--font-danger">{{$showErrorMsg}}</span></p>
                           </div>
                        @endif
                        @if(isset($successMsg))
                        <div class="form-group m-form__group">
                           <p style="text-align: center;"><span class="m--font-danger">{{$successMsg}}</span></p>
                        </div>
                        <div class="m-login__form-action">
                           <a href="{{url('/login')}}" class="btn btn-focus m-btn m-btn--pill m-btn--custom m-btn--air  m-login__btn m-login__btn--primary">Login In</a>
                        </div>
                        @else
                        <div class="form-group m-form__group">
                        ...
                        <div class="m-login__form-action">
                           <button id="m_login_signin_submit" class="btn btn-focus m-btn m-btn--pill m-btn--custom m-btn--air  m-login__btn m-login__btn--primary">Registe</button>
                           <a href="{{url('/login')}}" class="btn btn-focus m-btn m-btn--pill m-btn--custom m-btn--air  m-login__btn m-login__btn--primary">Login In</a>
                        </div>
                        @endif
                     </form>
